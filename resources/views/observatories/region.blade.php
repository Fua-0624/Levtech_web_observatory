    ...
    <div class="kakomi-tape">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <td><a href="{{ $observatory->HP_link }}"><font color="red">{{ $observatory->observatory }}</font></a></td>
                <td>{{ $observatory->hotel }}</td>
                <td>{{ $observatory->planetarium }}</td>
                <td>〒{{ $observatory->address_number }}<br><a href="{{ $observatory->google_url }}"><font color="green">{{ $observatory->address }}</font></a></td>
                <td>@if( $observatory->threads->count() != 0 )
                    <a href="/observatories/{{ $observatory->id }}/threads"><font color="blue">あり</font></a>
                    @else
                    <p>なし</p>
                    @endif
                </td>
